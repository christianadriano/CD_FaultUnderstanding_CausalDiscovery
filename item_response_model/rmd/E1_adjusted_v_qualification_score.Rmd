...
In order to pick the appropriate statistical test, I tested if the distributions are normal. For that I used the Shapiro-Wilk normality test (@royston1995shapiro).
...
```
The two distributions rejected the null hypothesis of the shapiro normality test, which means that one cannot guarantee that they are normal.

The null hypothesis of Shapiro's test is that the population is distributed normally. So, because we rejected the null-hypothesis (p-value<0.05), then the distribution is not normal. Hence, I chose the non-parametric test Kruska-Wallis 

I compared their means using the non-parametric Kruskal-Wallis test.


```{r hypotheses tests, echo=TRUE}
kruskal.test(df_consent$qualification_score,df_consent$adjusted_score)
```
