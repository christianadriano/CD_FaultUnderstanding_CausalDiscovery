...
Do people with the same experience agree in terms of the difficulty of the test items?

1- Did people with different levels of experience get the same test items correct?
2- If not, did they agree on the difficulty items?
3- Were the items equally discriminative across groups?
"
...
df <- df_E1[complete.cases(df_E1[,"qualification_score"],
                           df_E1[,"years_programming"]),] #left with 486 rows
...
"
Segment participants by years of programming experience. I used a 10-years period.
"

df_0_10 <- df[df$years_programming<=10,] %>% dplyr::select(test1_,test2_,test3_,test4_)
df_10_20 <- df[df$years_programming<=20 & df$years_programming>10,] %>% dplyr::select(test1_,test2_,test3_,test4_)
df_20_ <- df[df$years_programming>20,] %>% dplyr::select(test1_,test2_,test3_,test4_)

df_10 <- df[df$years_programming<=10,] %>% dplyr::select(test1_,test2_,test3_,test4_)

median(df$years_programming)
mean(df$years_programming)

df_below_3 <- df[df$years_programming<=3,] %>% dplyr::select(test1_,test2_,test3_,test4_)
df_above_3 <- df[df$years_programming>3,] %>% dplyr::select(test1_,test2_,test3_,test4_)

ltm(df_below_3 ~ z1, IRT.param=TRUE)
ltm(df_above_3 ~ z1, IRT.param=TRUE)

plot(df_below_3, type="ICC")

#------------------------------------------------------
"Run the 2PL model, only difficulty and discrimination"

ltm(df_0_10 ~ z1, IRT.param=TRUE)
#       Dffclt    Dscrmn
# test1_   0.571   0.637
# test2_   3.268  -0.382 *
# test3_  -0.043   8.042
# test4_   0.830  -1.285
ltm(df_10_20 ~ z1, IRT.param=TRUE)
# test1_  -0.400   1.757
# test2_  -1.841  14.586
# test3_  11.988  -0.120 *
# test4_   0.293  -0.579
ltm(df_20_ ~ z1, IRT.param=TRUE)
# test1_   0.448    0.736
# test2_   1.083  -37.522 *
# test3_   0.636  -13.261
# test4_  -0.059   17.760
